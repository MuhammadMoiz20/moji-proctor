 ...
 * @returns Token pair
 */
export async function generateTokenPair(fastify, prisma, userId) {
 ...
 */
export async function verifyRefreshToken(prisma, token, fastify) {
 ...
 */
export async function revokeRefreshToken(prisma, token) {
    await prisma.refreshToken.deleteMany({
        where: { token },
    });
}
